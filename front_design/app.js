...
(function () {
  ...
  function socialLinks(map, cls) {
    return Object.keys(map).filter(function (k) { return ICON[k]; }).map(function (k) {
  ...
  var btn = $('#an-ticket'); btn.firstChild ? null : null;
  btn.childNodes[0].nodeValue = a.ticketLabel + " ";
  btn.href = a.ticketUrl;
  var facts = [
    ['DATE', a.date], ['DOORS', a.doors], ['CITY', a.city], ['VENUE', a.venue]
  ];
  $('#an-facts').innerHTML = facts.map(function (f) {
  ...
  if (anLineEl) {
    anLineEl.innerHTML = anLineup.map(function (act, i) {
      ...
      return '<li class="ln-act' + (head ? ' ln-act--head' : '') + '">' +
        ...
        (tag ? '<span class="ln-tag">' + esc(tag) + '</span>' : '') +
        '</li>';
    }).join("");
  }
  ...
  function autoSlide() { ctimer = setInterval(function () { goSlide(ci + 1); }, 4200); }
  dotEls.forEach(function (d) { d.addEventListener('click', function () { clearInterval(ctimer); goSlide(+d.dataset.i); autoSlide(); }); });
  ...
  /* ============================================================
     MEDIA PLAYER — simulated playback (audio is mocked)
     ============================================================ */
  var sets = C.sets || [];
  var P = {
    i: parseInt(localStorage.getItem('barrel23_track') || '0', 10) || 0,
    t: parseFloat(localStorage.getItem('barrel23_pos') || '0') || 0,
    playing: false, raf: 0, last: 0
  };
  if (P.i >= sets.length) P.i = 0;

  var playerEl = $('#player');
  var elTitle = $('#pl-title'), elSub = $('#pl-sub'), elCur = $('#pl-cur'), elEnd = $('#pl-end');
  var seek = $('#seek'), fill = $('#seek-fill'), head = $('#seek-head'), btn2 = $('#pl-play');
  ...
  function setPlaying(on) {
    ...
    btn2.innerHTML = on ? iconPause : iconPlay;
    if (on) { P.last = performance.now(); loop(); } else { cancelAnimationFrame(P.raf); save(); }
  }
  function loop() {
    P.raf = requestAnimationFrame(function (now) {
      ...
      P.t += dt;
      var st = curSet();
      if (P.t >= st.duration) { P.t = 0; next(); return; }
      renderProgress();
  ...
  function next() { load(P.i + 1, P.playing); }
  function prev() { if (P.t > 3) { P.t = 0; renderProgress(); save(); } else load(P.i - 1, P.playing); }
  function save() {
    localStorage.setItem('barrel23_track', P.i);
    localStorage.setItem('barrel23_pos', P.t.toFixed(1));
  }

  btn2.addEventListener('click', function () { setPlaying(!P.playing); });
  $('#pl-next').addEventListener('click', next);
